...
import random
import math
db = SqliteDatabase("netlify.sqlite")
...
def write_file(new_contents):
    with open("README-netlify.md") as f:
        #去除标题
        for _ in range(7):
            f.readline()
    ...
    with open("README-netlify.md", "w") as f:
        f.write(new)


def craw_all():
    # 这是爬取所有的,github api限制每分钟请求最多30次
    api = "https://api.github.com/search/repositories?q=netlify-{}&sort=updated&per_page=100&page={}"
    item_list = []
...
def get_info(year):
    ...
    try:
        ...
        api = "https://api.github.com/search/repositories?q=netlify-{}&sort=updated".format(
            year)
        # 请求API
